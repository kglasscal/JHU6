...
### Generate Sample Mean Stats

```{r generateSampleMeansStats, echo=FALSE}

generateSampleMeansStats <- function (sampleRepetition, sampleMeansLists)
{
  if(VERIFY_SAMPLE_STATS_SCR | FALSE)
  {
    print("generateSampleMeansStats: verify inputs")
    print(paste0("mu           = ", mu))
  ...
  colnames(sampleStats) <- c("NRep", "Mean", "Mean\nAbsolute\nError", 
                             "Variance", "Variance\nAbsolute\nError")
  # -----------------------------------------------------------------------------
...
if(RUN_VERIFY_STATS)
{
  ...
  sampleMeansStats  <- generateSampleMeansStats(sampleRepetition, sampleMeansLists, mu, var)
...
imageMeanDistribution <- function(sampleRepetition, sampleMeansLists, binRange)
{
  ...
  for (i in 1:length(sampleRepetition)) {
    ...
    pltList[[i]] <- ggplot(frame, aes(x = pdf)) +
      geom_histogram(aes(y = after_stat(density)), breaks = binRange, fill = "#ffc845",
                     color = "#e9ecef") +
      geom_vline(aes(xintercept=mean(pdf)), color="#000000", linewidth = 0.5) +
      geom_vline(aes(xintercept = 0.0), color="#00bfb2", linewidth = 0.5) +
      geom_density(alpha = 0.2, color = "#000000", fill="#000000")  +
      stat_function(fun = function (x)
        dnorm(x, mean =  0, sd = 1),
        color = "#00bfb2", fill = "#00bfb2",
                geom = "area", alpha = 0.2) +
...
generateConvergeTable <- function (sampleMeansStats)
{
  ...
  resTable <- knitr::kable(sampleMeansStats, "latex",
    caption = paste0("table"),
    digits = 3, align = "llll",
    # col.names = c("NObs", "Mean", "Mean\nError", "Var", "Var Error"), 
    row.names = TRUE
    ) %>%
...
if(EXECUTE & TRUE)
{
  ...
  sampleMeansStats<- generateSampleMeansStats(sampleRepetition, sampleMeansLists)
  sampleStats      <- generateSampleMeansStats(sampleRepetition, sampleMeansLists)
  imageConvergencePlot(sampleRepetition, sampleStats)
...
To demonstrate the effect of the CLT on the sample means, document code will generate images using the $\textbf{\textit{imageMeanDistribution}}$ function. The function produces plots of the sample means distributions using the $\textit{sampleMeansLists}$ generated in the $\textbf{\textit{generateSampleMeansLists}}$. See Figure 1 a-d.
...
Each plot has three overlays. The first overlay is a histogram of the $\textit{sampleMeansLists}$. Figure 1 a-d displays the density of the sample means as yellow bars. Since the CLT should produce a Gaussian distribution from the sample means, the values in each sample means list are converted to random variables from a standard normal distribution using the z-score transformation (eqn 1).

\begin{equation}
\frac {\bar {X_n} - \mu}{\sigma_{\bar X_n}} =  \frac {X_n - 1/\lambda} {\sigma_{40}} =  \frac {\bar {X_n} - 5} {\sigma_{40}}.
\end{equation}

The second overlay is an approximation of the probability density function (pdf) shown as a black line filled 
...
imageMeanDistribution <- function(sampleRepetition, sampleMeansLists, binRange)
{
  ...
  for (i in 1:length(sampleRepetition)) {
    ...
    pltList[[i]] <- ggplot(frame, aes(x = pdf)) +
      geom_histogram(aes(y = after_stat(density)), breaks = binRange, fill = "#a0a0a0",
                     color = "#e9ecef") +
