...
imageMeanDistribution <- function(sampleRepetition, sampleSize, meanDistributions, binRange, binName)
{
  ...
  for (i in 1:length(sampleRepetition)) {
    frame <- NULL
    counts <- binCounts(x = meanDistributions[[i]], bx = unlist(binRange))

    frame  <- data.frame(pdf = meanDistributions[[i]], range = binRange[1:length(binRange)-1])
    colnames(frame) <- c("pdf", "range")
    # z = as.factor(unlist(binName)), pdf = counts/sampleRepetition[i] ) , name = as.factor(unlist(binName))
  
    if(RUN_MEAN_DIST_IMG & VERIFY_MEAN_DIST_IMG & TRUE)
    {
      # print(paste0("imageMeanDistribution: len counts:      ", length(counts)))
      ...
      # print(paste0("imageMeanDistribution: len counts:      ", counts))
    ...
    #   #   color = "green") + 
    #   after_stat(density)??????breaks   = unlist(binRange)
